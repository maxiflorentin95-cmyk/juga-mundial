...
router.post('/login', (req, res) => {
  ...
  const user = db.prepare('SELECT * FROM usuarios WHERE username=? OR email=?').get(username, username);
...
router.post('/register', (req, res) => {
  ...
  const result = db.prepare('INSERT INTO usuarios (username, email, password_hash) VALUES (?,?,?)').run(username, email, hash);
