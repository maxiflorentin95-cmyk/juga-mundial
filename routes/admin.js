...
function calcularPuntos(pL, pV, rL, rV) {
  ...
  if ((pL - pV) === (rL - rV))                     return 3;  // diferencia exacta
  if (Math.sign(pL - pV) === Math.sign(rL - rV))   return 2;  // resultado correcto
  return 0;
}

function calcularBonus(predGL, predGV, predClasifId, realClasifId, realGL, realGV) {
  if (parseInt(predGL) !== parseInt(predGV)) return 0;  // solo si predijo empate
  if (!predClasifId || !realClasifId) return 0;
  if (parseInt(predClasifId) !== parseInt(realClasifId)) return 0;
  // Bonus solo si el resultado real también fue empate en 90'
  return parseInt(realGL) === parseInt(realGV) ? 1 : 0;
}
...
router.post('/resultado', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    for (const pr of prons.rows) {
      ...
      const bonus = esEliminatoria ? calcularBonus(pr.goles_local, pr.goles_visitante, pr.clasificado_id, realClasifId, gl, gv) : 0;
  ...
  try {
    ...
    for (const partido of partidos) {
      ...
      for (const pr of prons) {
        const base  = calcularPuntos(pr.goles_local, pr.goles_visitante, partido.goles_local, partido.goles_visitante);
        const bonus = calcularBonus(pr.goles_local, pr.goles_visitante, pr.clasificado_id, partido.clasificado_id);
        const pts   = base + bonus;
        await client.query('UPDATE pronosticos SET puntos_obtenidos=$1 WHERE id=$2', [pts, pr.id]);
        actualizados++;
      }
...
router.post('/recalcular-partido', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    for (const pr of prons) {
      ...
      const bonus = esElim ? calcularBonus(pr.goles_local, pr.goles_visitante, pr.clasificado_id, p.clasificado_id) : 0;
      const pts   = base + bonus;
      await client.query('UPDATE pronosticos SET puntos_obtenidos=$1 WHERE id=$2', [pts, pr.id]);
    }
    await client.query(`
...
router.post('/cargar-pronostico-manual', requireAdmin, async (req, res) => {
  try {
    ...
    if (partido.estado === 'finalizado' && partido.goles_local !== null) {
      ...
      const bonus = esElim ? calcularBonus(gl, gv, clasifId, partido.clasificado_id) : 0;
      pts = base + bonus;
    }
...
router.post('/recalcular-clasificado', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    for (const pr of prons) {
      ...
      const base  = calcularPuntos(pr.goles_local, pr.goles_visitante, gl, gv);
      const bonus = calcularBonus(pr.goles_local, pr.goles_visitante, predClasifId, realClasifId, gl, gv);
      const pts   = base + bonus;
